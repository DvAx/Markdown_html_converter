...
parser = argparse.ArgumentParser(description="Programme de conversion de code markdown en HTML")
parser.add_argument('-i','--input-directory', help="Dossier source contenant les fichiers markdowns qui seront convertis")
parser.add_argument('-o', '--output-directory', help='Dossier destinataire qui va contenir les fichiers HTML qui seront générés')
    ...
    with open(fichier_markdown, 'r', encoding='utf-8') as mon_fichier_markdown:
        ...
        for ligne in mon_fichier_markdown:
            ...
            if 'http' in ligne:
                ...
                print("EXPRESSION URL :", adresse_url, "INDEX FIN URL :", index_fin_url, "LAST CARAC :", ligne[index_fin_url])
            ...
            if '*' in ligne:
                ...
                # Mot important
                etat_balise=False # Booléen qui représentera l'état de la balise à mettre (False : ouvrante - True : fermante)
                while '*' in ligne:
...
for nom_fichier, contenu_fichier in fichiers_html.items():
    nom_fichier=str(nom_fichier)[str(nom_fichier).find('/') + 1:str(nom_fichier).find('.')]+'.html'
